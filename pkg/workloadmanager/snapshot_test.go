...
func TestComputeSnapshotHash_Deterministic(t *testing.T) {
	...
	h1, err := computeSnapshotHash(ss, podSpec, sc)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	h2, err := computeSnapshotHash(ss, podSpec, sc)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if h1 != h2 {
...
func TestComputeSnapshotHash_TolerationOrderIndependent(t *testing.T) {
	...
	h1, _ := computeSnapshotHash(ss, corev1.PodSpec{Tolerations: []corev1.Toleration{tolA, tolB}}, sc)
	h2, _ := computeSnapshotHash(ss, corev1.PodSpec{Tolerations: []corev1.Toleration{tolB, tolA}}, sc)
	if h1 != h2 {
...
func TestComputeSnapshotHash_TolerationNilVsPtrZeroSeconds(t *testing.T) {
	...
	h1, _ := computeSnapshotHash(ss, corev1.PodSpec{Tolerations: []corev1.Toleration{nilSec, zeroSec}}, sc)
	h2, _ := computeSnapshotHash(ss, corev1.PodSpec{Tolerations: []corev1.Toleration{zeroSec, nilSec}}, sc)
	if h1 != h2 {
...
func TestComputeSnapshotHash_NodeNameIgnored(t *testing.T) {
	...
	h1, _ := computeSnapshotHash(ss, base, sc)
	h2, _ := computeSnapshotHash(ss, *withNode, sc)
	if h1 != h2 {
