...
```text
Create Sandbox -> wait until Ready -> serve requests -> TTL / idle GC
```
...
- requiring users to create `SandboxTemplate`, `SandboxClaim`, `SnapshotBuildTask`, or
...
| Scheduling | Built independently on multiple nodes | Usually tied to artifact locality |
...
Resume creates a new snapshot object for each saved session state, while artifact cleanup
is handled by system-level lifecycle policy.

---

## 2. Design Principles

### 2.1 The Snapshot Control Plane Must Not Enumerate Business Runtime Types
...
`SandboxSnapshotController` should not model those business runtime kinds directly.
Otherwise every new runtime type would require changes to the SandboxSnapshot schema,
webhook, controller logic, or adapter registry.
...
### 2.2 The Control Plane Must Not Call a VMM-Specific API

The AgentCube control plane does not call runtime-specific or VMM-specific APIs. Snapshot
operations are dispatched through internal `SnapshotBuildTask` resources:

```text
SandboxSnapshotController
-> create SnapshotBuildTask with standard spec
-> node agent watches tasks assigned to its node
-> node-local SnapshotDriver calls the local runtime / VMM
```

### 2.3 SnapshotBuildTask Is the Node Task Carrier

`SnapshotBuildTask` is an internal CRD created by `SandboxSnapshotController`. It carries:

- the build task declaration;
- target-node binding;
- the target sandbox reference;
- provider, artifact key, and build hash;
- node-agent-reported artifact phase and URI.
...
node agent watches `SnapshotBuildTask` objects and filters tasks assigned to its node by
`spec.targetNodeName`.

### 2.4 Artifact Deletion Is Eventually Consistent

Phase 1 uses `artifactLocality=NodeLocal`, so artifact deletion is eventually consistent.
When a `SandboxSnapshot` is deleted or its artifact records are replaced:

```text
control plane removes artifact records first
-> new sessions stop using old artifacts immediately
-> node-agent GC eventually removes local artifacts
```

The control plane does not synchronously wait for every node to confirm physical deletion.
Future remote artifact locality modes may use their own cleanup mechanism. A node-level
report resource can be introduced later if strict deletion auditability becomes a
requirement.
...
| Watches Snapshot / Template /      |   controller     | Reads Ready artifacts       |
| Sandbox / Node / BuildTask         |   is sole writer | Injects restore intent      |
| Builds artifacts and status        |                  | Adds restore intent         |
+------------------------------------+                  +-----------------------------+
              |                                                   |
...
| Watches SnapshotBuildTask          |                  | Consumes restore intent     |
| Runs SnapshotDriver and artifact GC|                  | Kuasar / Kata / others      |
+------------------------------------+                  +-----------------------------+
              |
              v
+------------------------------------+
...
1. watches `SandboxSnapshot`, `SandboxTemplate`, `Sandbox`, Node, and `SnapshotBuildTask`;
2. reads standard `SandboxTemplate` resources for `Fork` mode and standard `Sandbox`
   resources for `Resume` mode;
3. calculates immutable build hashes;
4. selects target nodes using `SnapshotClass.nodeSelector` and source scheduling
   constraints;
5. creates `SnapshotBuildTask` resources for nodes without valid artifacts;
6. watches `SnapshotBuildTask.status` and writes the artifact store;
7. aggregates `SandboxSnapshot.status`;
8. removes artifacts after build-hash drift, forced rebuild, or controller-detected
...
- call Kuasar, Kata, or other runtime APIs directly;
- synchronously delete local or remote snapshot artifacts;
- parse `CodeInterpreter`, `BrowserRuntime`, or other business CRDs.
...
2. watches `SnapshotBuildTask` resources targeted at its node;
3. selects an in-process driver using `SnapshotBuildTask.spec.providerName`;
4. creates local artifacts through the runtime / VMM;
5. writes local artifact phase and URI back to `SnapshotBuildTask.status`;
6. enumerates local artifacts and performs eventual GC;
7. validates artifact owner UIDs to prevent stale reuse after object recreation;
8. does not participate in the session restore data path.

The node agent must not patch `SandboxSnapshot.status`. Concurrent node-agent writes to
one object would create resource-version conflicts, unclear field ownership, and broader
RBAC permissions. `SandboxSnapshotController` is the only status writer.
...
`SnapshotBuildTask` remains the Kubernetes task API. The node-agent reconciler watches
that CRD, validates provider and target state, and converts it into
`SnapshotDriverCreateRequest` before calling the in-process driver. Drivers should not
read Kubernetes objects or depend on `SnapshotBuildTask` metadata/status directly.
...
other runtime APIs. The AgentCube control plane does not understand those private
protocols.

`ProviderName` comes from `SnapshotBuildTask.spec.providerName`. The node agent uses it to
...
- create: node agent watches a `SnapshotBuildTask`, then invokes a driver;
...
restore by the Kubernetes runtime compatibility layer. The node agent does not participate
in the session restore data path.
...
- `rebuildAfter`: rebuild after the baseline has remained Ready for this duration.
...
creates another `SandboxSnapshot(snapshotMode=Resume)` object. System-level cleanup
controls artifact GC. If `forkPolicy` is set when `snapshotMode=Resume`, the admission
webhook rejects the object.
...
`snapshotMode=Fork` with `artifactLocality=NodeLocal`.

`SandboxSnapshot.spec.snapshotMode` must be included in
`SnapshotClass.spec.supportedModes`. The admission webhook also rejects unsupported
...
remote provider service. `artifactLocality=NodeLocal` means the artifact is usable only on
the node that created it; it does not imply artifact distribution or replication. New
providers and artifact locality modes can be added later without exposing VMM-specific
settings to business users.

`supportedModes` is a capability set because `SandboxSnapshot.spec.snapshotMode` selects
the actual usage mode. `artifactLocality` remains a single value because it selects one
artifact placement and cleanup strategy for the class. If the same provider later supports
remote artifacts, define another class such as `kuasar-remote` instead of mixing multiple
locality strategies in one class.
...
type SnapshotArtifactManifest struct {
    ...
    // ArtifactSets maps ArtifactKey to the corresponding artifact set.
    // The map key is generated by SandboxSnapshotController and must match
    // SnapshotArtifactSet.ArtifactKey.
...
- `ArtifactSets` maps `ArtifactKey` to the corresponding artifact set, so one node can
  temporarily hold multiple artifacts that belong to different logical versions;
- `ActiveSetRef` and `PendingSetRef` point to entries in `ArtifactSets` by `ArtifactKey`;
- `Artifacts` contains the concrete artifacts in the same artifact set. For `NodeLocal`
  artifacts, `NodeName` is required and each node has at most one artifact in the same
  `SnapshotArtifactSet`;
- `ArtifactKey` is generated by `SandboxSnapshotController` and identifies a logical
  snapshot version shared across nodes. It should be human-readable when possible, for
  example `<normalized-snapshot-name>-<mode>-g<generation>-r<rebuildSeq>`. It is useful for
  logs, debugging, and grouping artifacts, but it is not the compatibility boundary.
  Snapshot UID, `BuildHash`, `ProviderName`, and `ProviderProtocolVersion` remain the
  authoritative validation inputs;
- `ArtifactURI` identifies the snapshot artifact in a runtime-consumable URI form. It must
  include a scheme. For `NodeLocal` artifacts, the standard logical URI is
  `local://snapshot/<artifact-key>`; the runtime compatibility layer resolves it to the
  node-local physical artifact on the node where the Sandbox is scheduled. Future remote
  artifact locality modes may use schemes such as `s3://`, `https://`, `oci://`, or
  `pvc://`. AgentCube treats the URI as opaque and passes it through. `ArtifactURI` is set
  only when `Phase=Ready`; for non-Ready phases, it must be empty;
...
- `ProviderProtocolVersion` records the provider build protocol version used to create the
  artifact and is validated before restore intent injection;
- `BuildHash` prevents restore from stale build inputs.
...
`SnapshotArtifactPhase` is shared by snapshot CRDs because it describes a node-local
...
report local artifact phases through `SnapshotBuildTask.status` and never write the
...
`ActiveSetRef.ArtifactKey` before injecting restore intent.
...
`ActiveSetRef.ArtifactKey` while populating `PendingSetRef.ArtifactKey`; a source
...
The following SandboxSnapshot flows use `activeSetRef.artifactKey` and
`pendingSetRef.artifactKey` as short names for the active and pending artifact set
references.

### 4.5 SnapshotBuildTask CRD

`SnapshotBuildTask` is an internal CRD. It is the node-facing task object for both `Fork`
...
`targetRef` is the sandbox to snapshot:

- `Fork`: a build `Sandbox` created by `SandboxSnapshotController` from the source
  `SandboxTemplate`;
- `Resume`: the existing source session `Sandbox`.

`SnapshotBuildTask.status.phase` is the node agent's report for the local snapshot
...
Build-stage protocol fields live on `SnapshotBuildTask`, not on the target `Sandbox`:

- build intent is stored in `SnapshotBuildTask.spec`;
- build result is stored in `SnapshotBuildTask.status`;
- the Fork build `Sandbox` is only the runtime target referenced by `targetRef`;
...
writes only `SnapshotBuildTask.status`. It does not write the artifact store or
...
`runtimeClassName` or the underlying runtime implementation. `snapshot-artifact-uri`
uses a runtime-consumable logical artifact URI. For `NodeLocal`, the value uses
`local://snapshot/<artifact-key>`. Kuasar-specific `kuasar.io/*` annotations belong only
inside a Kuasar compatibility layer.

On a session `Sandbox`, the presence of `snapshot-artifact-uri` is the restore intent.
Build results use `SnapshotBuildTask.status.artifactURI` instead. Restore compatibility is
defined by the runtime compatibility layer and the artifact URI scheme.
...
2. Computes buildHash and creates a new pending artifact set reference.
...
8. Creates one SnapshotBuildTask per build Sandbox.
9. Sets SnapshotBuildTask.targetRef to the build Sandbox.
```
...
2. Computes buildHash from the source Sandbox identity, runtime-relevant spec, and provider protocol.
3. Selects the node that currently hosts the Sandbox.
4. Creates one SnapshotBuildTask for that node.
5. Sets SnapshotBuildTask.targetRef to the source Sandbox.
...
```text
1. Watches SnapshotBuildTask where spec.targetNodeName=<this-node>.
2. Reads task.spec.targetRef.
...
6. Reports artifact URI and artifact phase on SnapshotBuildTask.status.
```

`SandboxSnapshotController` observes `SnapshotBuildTask.status`:
...
4. Deletes completed SnapshotBuildTasks.
5. Deletes temporary Fork build Sandboxes after their tasks complete.
```
...
3. Resolves manifest.ActiveSetRef.ArtifactKey to the active SnapshotArtifactSet.
4. Confirms the active set has at least one Ready artifact.
5. Validates snapshot UID, provider name, provider protocol version, artifact key, and build hash.
6. Injects restore intent using the active set's logical artifact URI if a valid artifact exists.
...
store. For `NodeLocal`, the `snapshot-artifact-uri` annotation uses
`local://snapshot/<artifact-key>`, and the runtime resolves the artifact on the scheduled
node.

Kubernetes runtime consumes restore intent:

```text
snapshot-artifact-uri exists
-> runtime attempts restore from ArtifactURI

snapshot-artifact-uri is absent
-> regular cold start
...
-> controller creates new build Sandboxes and SnapshotBuildTasks
-> new artifacts become Ready
-> sessions start restoring from new artifacts
-> node-agent GC removes old artifacts
```
...
Node Agent local GC
-> enumerates artifact owners
-> detects that the SandboxSnapshot UID no longer exists
-> eventually removes local artifacts
```
...
-> controller creates PendingSetRef, build Sandboxes, and SnapshotBuildTasks
...
## 6. Build Hash and Version Validation

`buildHash` identifies the inputs that affect artifact contents or restore compatibility.
...
The controller serializes `BuildHashInput` with a stable JSON serializer and computes:
...
- do not dereference Secret, ConfigMap, PVC, or projected volume contents. Hash only the
  references present in the pod template spec.

`buildHash` is a restore-compatibility boundary; it is not a hash of the captured runtime
memory state.
...
|---|---|
| `SandboxSnapshotController` | Creates build tasks and aggregates artifact phases; understands no private runtime protocol |
...
`SandboxSnapshot` declares the snapshot source only. It does not declare the runtime's
internal snapshot timing. The selected `SnapshotDriver` decides when the runtime is ready
for snapshot creation. A new runtime should not require changes to the CRD or
`SandboxSnapshotController`.
...
artifact URI during Pod sandbox creation. When the annotation is absent, the runtime follows
the regular cold-start path. This annotation is scoped to session `Sandbox` restore intent;
build results use `SnapshotBuildTask.status.artifactURI`. AgentCube treats the artifact URI
as opaque; URI scheme support is defined by the runtime compatibility layer. For
`NodeLocal` artifacts, the standard logical URI is `local://snapshot/<artifact-key>`.

Business-session context, if any, is outside the standard restore intent and is handled by
the business runtime integration.

### 7.4 Runtime Compatibility Layer
...
`snapshot-artifact-uri` as the runtime-consumable artifact URI. For `NodeLocal`, the
compatibility layer resolves `local://snapshot/<artifact-key>` on the node where the
Sandbox is scheduled, then restores, cold-starts, or fails according to the runtime's
restore policy. Compatibility is defined by the runtime compatibility layer and the
artifact URI scheme. During Pod sandbox creation, the compatibility layer uses the Sandbox
annotations and runtime-local state. AgentCube does not call a restore API and does not
define an in-process restore interface.
...
### 7.5 Adding a Non-Kuasar Runtime
...
Adding a runtime keeps `SandboxSnapshot`, the `SnapshotBuildTask` API, artifact records,
...
The later combination still uses the standard restore intent annotation. For `NodeLocal`
artifacts, the runtime compatibility layer resolves `local://snapshot/<artifact-key>` on
the node where the refill Sandbox is scheduled.
...
`SnapshotClass.nodeSelector` uses these labels. Node Agent must reject a build task when
the selected provider implementation's `SnapshotDriverCapabilities.ProviderProtocolVersion` does not match
`SnapshotClass.spec.providerProtocolVersion`.

Provider labels are discovery hints, not compatibility proofs. The controller and node
agent still validate provider name, provider protocol version, snapshot mode, and artifact
locality against `SnapshotClass` and `SnapshotDriverCapabilities`.
...
3. support the selected provider name, provider protocol version, snapshot mode, and
   artifact locality;
...
For `NodeLocal` artifacts, the restore intent URI is `local://snapshot/<artifact-key>`.
The runtime compatibility layer resolves that logical URI on the scheduled node. If the
node has the corresponding local artifact, the runtime can restore from it; otherwise the
runtime follows its restore policy, such as cold start or fail.

`SnapshotArtifact.NodeName` remains part of the artifact store for per-node status
aggregation and GC. Session placement stays with the Kubernetes scheduler.

---

## 10. Artifact GC

### 10.1 SnapshotArtifact Owner Metadata

Snapshot artifacts record:
...
The namespace is part of the artifact store key and GC lookup context; it is not duplicated
inside the Kubernetes `OwnerReference` value.

For `NodeLocal` artifacts, the metadata is also stored with the node-local physical
artifact so the node agent can match runtime artifacts with control-plane records. For
remote artifact locality modes, the same owner metadata is carried in the artifact record
or remote backend metadata according to the provider contract.

### 10.2 NodeLocal Artifact GC

Node Agent periodically scans node-local driver artifacts:

```text
lists local driver artifacts
-> checks whether the snapshot owner UID still exists
-> checks whether a referenced artifact record still exists under a retained version
-> deletes local artifacts without a valid owner or artifact record
-> applies a grace period to avoid deleting artifacts not yet persisted by the controller
```

Remote artifact physical cleanup is provider-specific. AgentCube removes artifact records
from the artifact store; the provider or remote backend performs eventual physical cleanup
according to its own retention and deletion contract.

### 10.3 Failure Cases

| Case | Handling |
|---|---|
| Controller crashes after artifact creation but before artifact-store write | Node-agent GC removes the untracked artifact after grace period |
| Snapshot owner deleted | Artifact records removed first; node-agent GC removes artifacts |
...
| Node permanently removed | Control plane removes artifact record; no physical cleanup wait |
| Build hash changes | Old artifact records removed immediately; old artifacts GC'd |
...
Pending
  |
  | create build Sandboxes and SnapshotBuildTasks
  v
Creating
  +-----------------+
...
and artifact.buildHash == current build hash
```
...
Pending
  |
  | create one SnapshotBuildTask on source node
  v
Creating
  +-----------------+
...
-> Controller collects task status
-> Controller deletes completed task
-> Fork: Controller deletes temporary build Sandbox
```
...
-> active artifact set available?
   ...
   + yes: inject restore intent with logical artifact URI
          -> runtime restore
...
| SnapshotBuildTask times out | Artifact build fails and retries with backoff |
| Driver readiness handshake is unsupported or fails | Artifact build fails; report driver-specific reason |
...
1. Snapshot-build artifacts contain runtime bootstrap state only. User state, active tasks,
   and session credentials stay outside reusable build artifacts. Phase 1 relies on the
   SnapshotDriver readiness handshake and build timing before user requests reach the
   runtime.
...
4. Node Agent processes only `SnapshotBuildTask` objects with
   `spec.targetNodeName=<this-node>`.
5. Before reporting `SnapshotBuildTask.status`, Node Agent validates:
   - SandboxSnapshot UID;
   - artifact key;
   - build hash;
   - driver;
   - target node.
6. Node Agent does not receive RBAC permission to update `SandboxSnapshot.status`.
7. Local artifact deletion validates owner UID to avoid deleting manually created or
   unrelated artifacts.

---

## 15. Differences From the Current Prototype

| Current prototype | Target design |
|---|---|
| `SnapStart` CRD | `SandboxSnapshot` CRD with `snapshotMode` |
| `runtimeRef -> CodeInterpreter / AgentRuntime` | `sourceRef -> SandboxTemplate` |
| SnapshotController parses CodeInterpreter | Business controllers generate templates; SandboxSnapshotController reads templates only |
| SnapshotController calls `agentd` Kuasar Proxy | Controller creates `SnapshotBuildTask`; Node Agent watches and reconciles |
| Control plane understands template ID, lease, and `kuasar.io/*` | Node-agent SnapshotDriver understands private protocol |
| `SnapStart.status.activeMode` | `SandboxSnapshot.status.phase` and node count fields |
| Kuasar-oriented Redis `SnapshotInfo` | Generic `SnapshotArtifactManifest`, `SnapshotArtifactSet`, and `SnapshotArtifact` |
| Workload Manager writes `kuasar.io/*` | Workload Manager writes AgentCube standard restore intent |
| Controller uses build Sandbox annotations as node task status | Internal `SnapshotBuildTask.status` |
| Controller synchronously cleans local or remote snapshot artifacts | Immediate artifact-record removal plus provider-specific eventual cleanup |
| CodeInterpreter creates `SandboxTemplate` only when `warmPoolSize > 0` | Runtime controller always reconciles a stable `SandboxTemplate`; `warmPoolSize` only controls `SandboxWarmPool` |

This proposal update does not modify code.

---

## 16. Delivery Plan

### Phase 1: Abstraction Refactor and Kuasar Integration
...
4. Add internal `SnapshotBuildTask` CRD and controller reconciliation.
5. Change `agentd` to watch node-targeted `SnapshotBuildTask` resources.
...
9. Implement node-local artifact GC.

### Phase 2: Generic AgentRuntime and Browser Agent

1. Make AgentRuntimeController generate `SandboxTemplate`.
2. Make BrowserRuntimeController generate `SandboxTemplate`.
3. Validate each driver's readiness handshake and restore injection contract.
4. Add new business runtimes without changing SandboxSnapshotController.

### Phase 3: WarmPool Combination
...
Implement `Resume` mode for session-state snapshots from existing session Sandboxes. Reuse
the same `SandboxSnapshot` CRD, controller, driver, artifact store, and restore intent.
Restore completion remains a data-plane action; GC remains a system-level artifact cleanup
concern.
...
1. Does `SnapshotClass` need capability status, or are Node labels enough for Phase 1?
2. Will agent-sandbox accept the AgentCube standard session restore annotation upstream, or
   should a temporary compatibility layer translate it?
3. Should the long-term artifact store remain Redis / Valkey or become a dedicated API?
4. How should node-agent GC query Snapshot owner UIDs with minimum RBAC scope?
